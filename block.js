...
class block{
  constructor(a,b,c,d,e,f,g,h){
    ...
    if(this.type == 1||this.type == 2||this.type == 6||this.type == 9||this.type == 10){
    ...
    if(this.type == 1||this.type == 6||this.type==9||this.type == 10){
  ...
  form1(x,y,sx,sy,a,b,c,d){
    ...
    for(let i=0; i<sx/30; i++){
      ...
      if(i!=0&&i!=(sx/30-1)&&c!=0&&c!=(sy/30-1)){
        if(rectHit(-trucam.x+500,-trucam.y+300,this.bx,this.by,1000*100/zoom,600*100/zoom,this.bsx,this.bsy)){
          image(tileFull[0][0],plot.x,plot.y,30-15+imageSize,30-15+imageSize)
        }
      }else{
        if(rectHit(-trucam.x+500,-trucam.y+300,this.bx,this.by,1000*100/zoom,600*100/zoom,this.bsx,this.bsy)){
        let seter = [
        ...
        for(let i=0; i<3; i++){
          for(let c= 0; c<3; c++){
            seter[i][c]=(blocks.findIndex(hold => rectHit(hold.bx,hold.by,plot.x-15+c*30,plot.y-15+i*30,hold.bsx,hold.bsy,20,20)&&((hold.type==this.type||hold.type==2||hold.type==3)&&hold.life>0))>=0)+0;
            seter2[i][c]=(blocks.findIndex(hold => rectHit(hold.bx,hold.by,plot.x-15+c*30,plot.y-15+i*30,hold.bsx,hold.bsy,20,20)&&(hold.type==this.type&&hold.life>0))>=0)+0;
        ...
        this.form3(plot.x,plot.y,seter,seter2,ava);
      } 
      }
      }
  }
  ...
  form3(x,y,seter,seter2,ava){
    ...
    if("bottom_right"=="bottom_right"){
      ...
      if(arrays2DEqual(plot2,plot1)&&ava[1][1]||arrays2DEqual(plot3,plot1)) setes[1][1] = createVector(3,0);
    ...
    let sizer = imageSize;
    image(tileSprite[setes[0][0].x][setes[0][0].y],x,y,sizer,sizer);
    image(tileSprite[setes[0][1].x][setes[0][1].y],x,y+15,sizer,sizer);
    image(tileSprite[setes[1][0].x][setes[1][0].y],x+15,y,sizer,sizer);
    image(tileSprite[setes[1][1].x][setes[1][1].y],x+15,y+15,sizer,sizer);
