...
class player{   
   ...
   // playersP[playersP.length] = new playerP(this.px,this.py,this.direction*random(1,2),-random(1,2),this.skinx);
    for(var i=0; i<blocks.length; i++){
      ...
      for(var u = 0; u<3; u++){
          ...
          for(var u = 0; u<20; u++){
        bulletsP[bulletsP.length] = new bulletP(blocks[i].bx,blocks[i].by,blocks[i].bx+cos(random(-360,0)*PI/180)*10,blocks[i].by+sin(random(-360,0)*PI/180)*10,random(3,25),1);
           }
        }
      }
      }
  ...
  respawn(){
   shake=100;
    ...
    bulletsP[bulletsP.length] = new bulletP(this.px,this.py,this.px+cos(angle)*50,this.py+sin(angle)*50,fast);
    }
    /*
